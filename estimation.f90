...
! modules
module module487    ! 声明程序名称，可以在其他代码重用
! implicit none ! 显式声明所有变量
contains            ! 声明接下来的内容

! 一维的mean计算
pure function mean1(x) ! pure function:不修改外部变量也不读写文件 
real , intent(in), dimension(:) :: x !定义输入参数x real 类型 intent(in)只读取x不修改  ，dimension待定
real  ::   mean1  !返回值定义
mean1=sum(x)/size(x) !元素总和除以元素个数
end function mean1 

pure function mean3(x)
real , intent(in), dimension(:,:,:) :: x !三维数组 x是三维时的求和
real  ::   mean3
mean3=sum(x)/size(x)
end function mean3
